...
import { useGetAllPostsQuery, useGetLikesQuery, useGetUserPostQuery } from "@/app/_lib/api/post";
...
export default function Account() {
  ...
  const { data: praisedPost, isLoading: praisedPostLoading } = useGetLikesQuery("");
  //@ts-ignore
  ...
  const sectionsClassName = " bg-white pt-2 xs:pt-3 sm:pt-4 pb-6 px:2 xs:px-3 sm:px-4 shadow-xl";

  const LikesContent = useMemo(() => (praisedPostArray || []).map((data: Post) => <PostDisplay {...(data as Post)} isAuthorPost={info?.id === data.author} className={sectionsClassName} />), [praisedPostArray, praisedPostLoading]);
  const UserPostContent = useMemo(() => (userPostArray || []).map((data: Post) => <PostDisplay {...(data as Post)} isAuthorPost className={sectionsClassName} />), [userPostArray, userPostLoading]);
    ...
    <PageContainer title="Account">
      <div className="flex flex-col gap-4">
        ...
        {!!extraSections?.length && (
          <section className={`${sectionsClassName} flex flex-col gap-4`}>
            {
