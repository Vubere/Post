...
import { FormState, RegisterOptions, UseFormGetValues, UseFormRegister, useWatch } from "react-hook-form"
...
export default function Input(props: Props) {
  ...
  return (
    <div>
      <div className="h-[40px] w-full border border-1 border-black border-opacity-70 rounded-[6px] px-2 py-1 text-sm text-black flex flex-col items-center justify-center relative overflow-hidden hide gap-0 [&_label]:text-[8px] [&.hide_label]:text-sm [&:hover_input]:opacity-100 [&:hvoer_input]:static [&.hide_input]:opacity-0 [&.hide_input]:absolute  [&:hover_input]:opacity-100 [&:hover_input]:static [&.hide_input]:z-[-3] [&.hide:hover_input]:z-[2] [&:hover_input]:block [&:hover_label]:text-[8px] [&:hover_label]:leading-[100%] " id={containerId} ref={containerRef as any}>
...
interface Props {
  ...
  name: string,
  placeholder?: string,
  state: { formState: FormState<any>, getValues: UseFormGetValues<any>, register: UseFormRegister<any>, control: any },
  required?: boolean,
  readonly?: boolean,
...
export function NormalInput(props: Omit<Props, "state">) {
  ...
  const renderInput = (className: string, type?: string,) => {
    switch (type) {
      case "text": {
        return (
          <input {...inputProps} className={className + " " + (inputProps.className || "")} {...overideHookForm} {...(extraProps || {})} id={name} />
        )
      }
      case "textarea": {
        return (
          <textarea {...inputProps} className={className + " leading-[105%] resize-none h-[67px] min-h-[67px] " + (inputProps.className || "")} {...overideHookForm} {...(extraProps || {})} id={name} />
        )
      }
      default: {
        return <input type={type} {...inputProps} className={className + " " + (inputProps.className || "")} {...overideHookForm} {...(extraProps || {})} id={name} />
      }
    }
  }
  ...
  return (
    <>
      <div className={`${type === "textarea" ? "h-[90px]" : "h-[40px] flex flex-col items-center justify-center gap-1"} w-full border border-1 border-black border-opacity-70 rounded-[6px] px-2 text-sm text-black  relative overflow-hidden hide gap-0 [&_label]:text-[8px] [&.hide_label]:text-sm [&:hover_input]:opacity-100 [&:hover_input]:static [&.hide_input]:opacity-0 [&.hide_input]:absolute  [&:hover_input]:opacity-100 [&:hover_input]:static [&.hide_input]:z-[-3] [&.hide:hover_input]:z-[2] [&:hover_input]:block [&:hover_label]:text-[8px] [&:hover_label]:leading-[100%] `} id={containerId} ref={containerRef as any}>
