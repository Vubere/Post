...
function Modal({ title, children, close, open, twHeight, twWidth, className }: ModalProp) {
  ...
  return (
    open ? (
      <div className={`fixed top-0 left-0 h-[100vh] rounded-xl  w-[100vw] translate-y-[-50%] translate-x-[-50%] max-h-[100vh] mx-auto px-2 sm:px-4 bg-[#fff] text-black overflow-y-auto py-4 pb-10 z-[20] ${className} ${height} ${width}`}>
        {title && (<p className="pb-4 border-b border-1 text-[16px] xs:text-[18px] sm:text-[21px] font-bold mb-4 ">{title}</p>)}

        <Button className="!rounded-full !ml-auto !w-[35px] !h-[35px] px-2 !border-none !outline-none !bg-transparent absolute right-[15px] top-[15px]" theme="light" onClick={close}>
